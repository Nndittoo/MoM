                ...
                <div class="task-card relative group bg-gray-800 rounded-xl border-l-4 {{ $borderColor }} shadow-md hover:shadow-2xl hover:shadow-red-500/10 transition-all duration-300 hover:-translate-y-1 overflow-hidden"
                    ...
                    <div class="p-5">
                        ...
                        <div class="flex flex-col space-y-2 mt-3 text-sm text-gray-400">
                            <span title="Asal MoM" class="flex items-center"><i class="fa-solid fa-file-lines w-4 mr-2 text-gray-500"></i>{{ \Str::limit($task->mom->title ?? 'N/A', 40) }}</span>
                            <span title="Deadline" class="flex items-center {{ $isOverdue ? 'text-red-400 font-semibold' : ($isDueSoon ? 'text-yellow-400 font-semibold' : '') }}">
                                <i class="fa-solid fa-calendar-day w-4 mr-2 text-gray-500"></i>{{ $deadline->translatedFormat('d F Y') }}
                            </span>
                        </div>
                    </div>
