...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function renderMonthlyEvents(month, year) {
            ...
            // Apply filter
            const filteredEvents = monthlyEvents.filter(event => {
                if (currentFilter === 'pending') return event.status === 'mendatang';
                if (currentFilter === 'completed') return event.status === 'selesai';
                return true;
            });
            ...
            if (filteredEvents.length > 0) {
                ...
                filteredEvents.forEach(event => {
                    ...
                    if (event.status === 'selesai') {
                        ...
                        statusBadge = '<span class="px-2 py-0.5 text-xs font-semibold bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300 rounded-full">Completed</span>';
                    } else if (deadlineDate < todayDate) {
                        deadlineClass = 'text-red-700 font-bold';
                        statusBadge = '<span class="px-2 py-0.5 text-xs font-semibold bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300 rounded-full">Overdue</span>';
                    } else if (deadlineDate.getTime() === todayDate.getTime()) {
                        deadlineClass = 'text-orange-500 font-bold';
                        statusBadge = '<span class="px-2 py-0.5 text-xs font-semibold bg-orange-100 text-orange-800 dark:bg-orange-900 dark:text-orange-300 rounded-full">Today</span>';
                    }
                    ...
                    listItem.innerHTML = `
                        ...
                        <div class="flex-grow">
                            ...
                            <a href="/admin/moms/${event.mom_id}" class="text-sm text-text-secondary dark:text-dark-text-secondary hover:text-primary hover:underline">
                                ${event.momTitle}
                            </a>
        ...
        document.querySelectorAll('.filter-btn').forEach(btn => {
            btn.addEventListener('click', function() {
                ...
                else if (filterId === 'filterPending') currentFilter = 'pending';
                else if (filterId === 'filterCompleted') currentFilter = 'completed';
