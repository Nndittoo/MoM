...
<div class="pt-2">
    ...
    <div class="p-6 md:p-8 rounded-xl shadow-lg bg-gray-800 border-l-4 border-red-500 mb-6">
        ...
        <p class="mt-1 text-gray-400">Kelola informasi akun dan foto profil Anda sebagai admin.</p>
    </div>
        ...
        <div class="lg:col-span-1">
            <div class="bg-gray-800 rounded-2xl shadow-lg border border-gray-700 p-6">
                ...
                <form action="{{ route('profile.photo.update') }}" method="POST" enctype="multipart/form-data" class="mt-6 space-y-4">
                    ...
                    <button class="w-full px-4 py-2 rounded-lg btn-neon-red text-white font-semibold" type="submit">Upload Foto</button>
                </form>
                ...
                <form id="delete-photo-form" action="{{ route('profile.photo.delete') }}" method="POST" class="mt-4">
                    @csrf
                    @method('DELETE')
                    <button id="delete-photo-button" type="button" class="w-full px-4 py-2 rounded-lg bg-transparent border border-red-500 text-red-400 hover:bg-red-500/10 font-semibold">Hapus Foto</button>
                </form>
                @endif
            </div>
        </div>
            ...
            <div class="bg-gray-800 rounded-2xl shadow-lg border border-gray-700 p-6">
                ...
                <form action="#" method="POST" class="space-y-4"> {{-- Ganti '#' dengan route update info Anda --}}
                    ...
                    <div>
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name', $user->name) }}" class="bg-gray-700 border border-gray-600 text-white text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2.5" required>
                    </div>
                    <div>
                        <label for="email" class="block mb-2 text-sm font-medium text-gray-300">Alamat Email</label>
                        <input type="email" id="email" name="email" value="{{ old('email', $user->email) }}" class="bg-gray-700 border border-gray-600 text-white text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2.5" required>
                    </div>
                </form>
            </div>
        </div>
            ...
            deleteButton.addEventListener('click', function (event) {
                ...
                Swal.fire({
                    ...
                    customClass: {
                        ...
                        confirmButton: 'btn-neon-red text-white font-semibold px-6 py-2 mr-2 rounded-lg',
