...
@section('content')
<div class="pt-2">
    {{-- Toast Notification (Tema Disesuaikan) --}}
    ...
    {{-- Header (Tema Disesuaikan) --}}
    <div class="p-4 md:p-6 rounded-xl shadow-lg bg-gray-800 border-l-4 border-red-500 mb-4">
        <h1 class="text-2xl md:text-3xl font-bold font-orbitron text-neon-red">Create New MoM</h1>
        <p class="mt-1 text-sm md:text-base text-gray-400">Buat notulensi rapat baru dengan mengisi form di bawah ini.</p>
    </div>

    {{-- Form Container (Tema Disesuaikan) --}}
    <div class="bg-gray-800 p-4 md:p-6 rounded-2xl shadow-lg border border-gray-700">
        <form id="mom-form" class="space-y-6">
            @csrf

            {{-- Informasi Rapat --}}
            <div class="space-y-4">
                <h2 class="text-base md:text-lg font-semibold text-white font-orbitron border-b border-gray-700 pb-2">Informasi Rapat</h2>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="judul" class="block mb-2 text-xs md:text-sm font-medium text-gray-300">Judul Rapat</label>
                        <input type="text" name="title" id="judul" class="bg-gray-700 border border-gray-600 text-white text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2" placeholder="Contoh: Rapat Progres Proyek Q3" required>
                    </div>
                    <div>
                        <label for="tempat" class="block mb-2 text-xs md:text-sm font-medium text-gray-300">Tempat</label>
                        <input type="text" name="location" id="tempat" class="bg-gray-700 border border-gray-600 text-white text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2" placeholder="Contoh: Ruang Rapat Lt. 5 / Online" required>
                    </div>
                    <div>
                        <label for="pimpinan_rapat" class="block mb-2 text-xs md:text-sm font-medium text-gray-300">Pimpinan Rapat</label>
                        <input type="text" name="pimpinan_rapat" id="pimpinan_rapat" class="bg-gray-700 border border-gray-600 text-white text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2" placeholder="Nama Pimpinan" required>
                    </div>
                    <div>
                        <label for="notulen" class="block mb-2 text-xs md:text-sm font-medium text-gray-300">Notulen</label>
                        <input type="text" name="notulen" id="notulen" class="bg-gray-700 border border-gray-600 text-white text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2" placeholder="Nama Notulen" required>
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label for="tanggal" class="block mb-2 text-xs md:text-sm font-medium text-gray-300">Tanggal</label>
                        <input type="date" name="meeting_date" id="tanggal" class="bg-gray-700 border border-gray-600 text-white text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2" required>
                    </div>
                    <div>
                        <label for="waktu_mulai" class="block mb-2 text-xs md:text-sm font-medium text-gray-300">Waktu Mulai</label>
                        <input type="time" name="start_time" id="waktu_mulai" class="bg-gray-700 border border-gray-600 text-white text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2" required>
                    </div>
                    <div>
                        <label for="waktu_selesai" class="block mb-2 text-xs md:text-sm font-medium text-gray-300">Waktu Selesai</label>
                        <input type="time" name="end_time" id="waktu_selesai" class="bg-gray-700 border border-gray-600 text-white text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2" required>
                    </div>
                </div>
            </div>
            ...
            {{-- Peserta & Agenda --}}
            <div class="space-y-4">
                <h2 class="text-base md:text-lg font-semibold text-white font-orbitron border-b border-gray-700 pb-2">Peserta & Agenda</h2>
                <div class="grid grid-cols-1 gap-4">
                    <div>
                        <label class="block mb-2 text-xs md:text-sm font-medium text-gray-300">Peserta Rapat (Per Unit/Bagian)</label>
                        <div class="flex flex-col md:flex-row gap-3 items-end">
                            <div class="w-full">
                                <input type="text" id="input-internal-unit" class="bg-gray-700 border border-gray-600 text-white text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2" placeholder="Nama Unit/Bagian (Contoh: Unit HC)">
                            </div>
                            <button type="button" id="btn-add-internal-unit" class="px-4 py-2 text-xs md:text-sm font-medium text-white btn-neon-red rounded-lg w-full md:w-auto flex-shrink-0">Tambah Unit</button>
                        </div>
                        <div id="list-internal-attendees-container" class="space-y-3 mt-3"></div>
                        <p class="mt-1 text-xs text-gray-500">Tambah Unit, lalu masukkan nama-nama peserta dari Unit tersebut.</p>
                    </div>
                </div>
                <div class="grid grid-cols-1 gap-4 pt-4 border-t border-gray-700">
                    <div>
                        <label class="block mb-2 text-xs md:text-sm font-medium text-gray-300">Agenda</label>
                        <div class="flex gap-2">
                            <input type="text" id="input-agenda" class="bg-gray-700 border border-gray-600 text-white text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2" placeholder="Ketik agenda lalu Enter...">
                            <button type="button" id="btn-add-agenda" class="px-4 py-2 text-xs md:text-sm font-medium text-white btn-neon-red rounded-lg">Add</button>
                        </div>
                        <ol id="list-agenda" class="mt-2 space-y-1 list-decimal list-inside text-xs md:text-sm text-gray-300"></ol>
            ...
            {{-- Pihak Luar (Mitra) --}}
            <div class="space-y-4">
                <h2 class="text-base md:text-lg font-semibold text-white font-orbitron border-b border-gray-700 pb-2">Pihak Luar (Mitra)</h2>
                <div class="flex flex-col md:flex-row gap-3 items-end">
                    <div class="w-full">
                        <label for="input-mitra-nama" class="block mb-2 text-xs md:text-sm font-medium text-gray-300">Nama Mitra/Instansi</label>
                        <input type="text" id="input-mitra-nama" class="bg-gray-700 border border-gray-600 text-white text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2" placeholder="Ketik nama Mitra">
                    </div>
                    <button type="button" id="btn-add-mitra" class="px-4 py-2 text-xs md:text-sm font-medium text-white btn-neon-red rounded-lg w-full md:w-auto flex-shrink-0">Tambah Pihak Luar</button>
                </div>
                <div id="list-mitra-container" class="space-y-3 mt-3"></div>
            ...
            {{-- Lampiran --}}
            <div class="space-y-4">
                <h2 class="text-base md:text-lg font-semibold text-white font-orbitron border-b border-gray-700 pb-2 mb-4">Lampiran</h2>
                <input class="block w-full text-xs md:text-sm text-gray-400 border border-gray-600 rounded-lg cursor-pointer bg-gray-700 focus:outline-none file:bg-gray-600 file:border-0 file:text-gray-300 file:px-3 file:py-2" id="lampiran-input" name="attachments[]" type="file" multiple>
                <p class="text-xs text-gray-500">PNG, JPG, PDF, DOCX (MAX. 10MB). Dapat mengunggah lebih dari 1 file.</p>
                <div id="file-list" class="mt-2 space-y-1 text-xs md:text-sm text-gray-400"></div>
            </div>

            {{-- Submit Button --}}
            <div class="flex justify-end gap-2 pt-4 border-t border-gray-700">
                <button type="button" id="btn-submit" class="text-white btn-neon-red btn-pulse font-semibold rounded-lg text-sm md:text-base px-8 py-2.5 text-center">
                    <i class="fa-solid fa-rocket mr-2"></i>Submit MoM
                </button>
            </div>
        </form>
    </div>
